...
RUN printf "NEXT_PUBLIC_WEB_URL=%s\nNEXT_PUBLIC_PWA_URL=%s\nNEXT_PUBLIC_DONATE_URL=%s\nVITE_NOTIFICATIONS_WS_URL=%s\nVITE_VAPID_PUBLIC_KEY=%s\nNEXT_PUBLIC_WEB_RYBBIT_SITE_ID=%s\nNEXT_PUBLIC_RYBBIT_SCRIPT_URL=%s\nOTEL_EXPORTER_OTLP_ENDPOINT=%s\nOTEL_EXPORTER_OTLP_HEADERS=%s\nOTEL_SERVICE_NAME=%s\n" \
    "$NEXT_PUBLIC_WEB_URL" "$NEXT_PUBLIC_PWA_URL" "$NEXT_PUBLIC_DONATE_URL" "$VITE_NOTIFICATIONS_WS_URL" "$VITE_VAPID_PUBLIC_KEY" "$NEXT_PUBLIC_WEB_RYBBIT_SITE_ID" "$NEXT_PUBLIC_RYBBIT_SCRIPT_URL" "$OTEL_EXPORTER_OTLP_ENDPOINT" "$OTEL_EXPORTER_OTLP_HEADERS" "$OTEL_SERVICE_NAME" > .env
