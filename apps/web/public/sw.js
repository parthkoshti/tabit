...
self.addEventListener("push", function (event) {
  const show = (function () {
    ...
    const url = decl?.navigate || (data.url || new URL(data.type === "tab_invite" ? "/app/tabs" : "/app/friends", self.location.origin).href);
    const options = {
      body,
...
self.addEventListener("notificationclick", function (event) {
  ...
  const url = event.notification.data?.url || new URL("/app/tabs", self.location.origin).href;
  event.waitUntil(
